...
| `api/state.js`                 | `GET` — full `{ ingredients, categories, recipes, mealPlan }` for the signed-in user. Also `POST`/`DELETE` for meal plan entries (assign/remove a recipe on a day of the week) — deliberately kept on this route rather than a separate `api/meal-plan.js` to stay at 12 Serverless Functions, Vercel's Hobby-plan cap (adding a 13th route function failed the deployment outright). POST validates `dayOfWeek` against a fixed Mon–Sun list and checks recipe ownership. |
...
| `api/recipes.js`               | `POST`/`PATCH`/`DELETE` — create/update/remove a recipe + its `recipe_ingredients`. PATCH accepts `id` plus optional `name` and/or `ings`, applied independently (a pure rename sends just `{id, name}`). Exports `ensurePantryIngredients(userId, ings)` (module-local, not exported outside the file) which auto-creates any referenced ingredient missing from the pantry, unchecked; both POST and PATCH return `newIngredients` from it. PATCH does an explicit `recipes` ownership check before touching `recipe_ingredients`, since that table has no `user_id` of its own. |
...
| `scripts/migrate-meal-plan.mjs` | Creates `meal_plan_entries` (`user_id`, `day_of_week`, `recipe_id`) + its user_id index (`npm run db:migrate-meal-plan`) |
...
| `src/useStore.js`          | `useStore()` — `ingredients`/`recipes`/`categories`/`mealPlan`/`loading`/`error` (fetches `/api/state` on mount, StrictMode-safe via an ignore-flag guard); optimistic-update mutators: `addIngredient`, `removeIngredient`, `toggleIngredient`, `setIngredientCategory`, `addCategory`, `renameCategory`, `removeCategory`, `addRecipe`, `removeRecipe`, `updateRecipeIngs`, `renameRecipe`, `addMealPlanEntry`, `removeMealPlanEntry` (the ings-touching ones merge `newIngredients` from the API into local state); pure `getMatch(recipe)`/`getSortedRecipes(skipSet)`/`getShoppingList()` (the latter derives entirely from `mealPlan`+`recipes`+`ingredients`, no API call of its own) |
...
| `src/Recipes.jsx`          | Recipes tab. Default `Recipes` + three file-local sub-components: `IngredientAutocomplete` (the custom-built, pantry-sourced suggestion dropdown — not a native `<datalist>` — shared by both usages below), `RecipeCard` (match %, `MatchBar`, tag chips; edit mode renames the recipe, renames/adds/removes ingredients, toggles essential, always keeping ≥1 ingredient; delete asks for confirmation), and `AddRecipeForm` (name + ingredient list via `IngredientAutocomplete`, free-text fallback for new ingredients). Splits recipes into makable (sorted by match % descending) / "Unmakable" via `getMatch(r).unmakable` |
...
| `src/MealPlan.jsx`         | Meal Plan tab. Default `MealPlan` + file-local `DaySection` (one per fixed Monday→Sunday day): lists that day's assigned recipes with a remove button, plus a native `<select>` recipe picker + "Add" button to assign more. Shows a guidance message instead if `recipes` is empty. |
| `src/MealPlan.module.css`  | Styles for day sections, entry rows, and the recipe-picker row                                                                                                                                                                                                                                            |
| `src/ShoppingList.jsx`     | Shopping List tab — purely a view over `store.getShoppingList()`; no local state or fetch of its own. Renders a checklist where checking an item calls the same `toggleIngredient` the Ingredients tab uses. Two distinct empty states: no meal plan entries at all vs. nothing currently missing. |
| `src/ShoppingList.module.css` | Styles for the checklist rows, mirroring `Ingredients.module.css`'s row/checkbox/label pattern                                                                                                                                                                                                        |
