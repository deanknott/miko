...
| `api/state.js`                 | `GET` — full `{ ingredients, categories, recipes, mealPlan }` for the signed-in user   |
...
| `api/meal-plan.js`             | `POST`/`DELETE` only — assign a recipe to a day of the week / remove an entry, scoped to `session.userId`; validates `dayOfWeek` against a fixed Mon–Sun list and checks recipe ownership on POST. No `GET` — entries are fetched as part of `/api/state`. |
