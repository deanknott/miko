...
- The category list itself is sorted alphabetically for display; ingredients within each category (and "Uncategorized") sort checked-first then alphabetically (`byCheckedThenName` in `Ingredients.jsx`) — this is display-only, not the underlying array order.
- The categories/ingredients section only falls back to the "No ingredients yet" empty state when **both** `ingredients` and `categories` are empty — checking `ingredients.length === 0` alone (the original condition) hid an already-created category (and its delete button) whenever it had zero ingredients in it yet, a real bug found via testing, not reported by a user.
- All three delete actions in the app (ingredient, category, recipe) go through a native `confirm()` prompt before calling the mutator — no custom modal component exists yet; `window.confirm` was the deliberate choice over building one, given the app's personal scale.
